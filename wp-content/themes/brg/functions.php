...
add_action('wpcf7_mail_sent', function ($cf7) {
	...
	if(isset($data['firstname']) && !empty($data['firstname'])){
		$txt .= "Ім'я: " . strip_tags(trim(urlencode($data['firstname']))) . "%0A";
	}
	if(isset($data['lastname']) && !empty($data['lastname'])){
		$txt .= "Прізвище: " . strip_tags(trim(urlencode($data['lastname']))) . "%0A";
	}
	if(isset($data['username']) && !empty($data['username'])){
		$txt .= "ПІБ: " . strip_tags(trim(urlencode($data['username']))) . "%0A";
	}
	...
	if (isset($data['company']) && !empty($data['company'])) {
		$txt .= "Назва компанії або ФОП: " . strip_tags(trim(urlencode($data['company']))) . "%0A";
	}
	if (isset($data['activity']) && !empty($data['activity'])) {
		$txt .= "Вид діяльності: " . strip_tags(trim(urlencode($data['activity']))) . "%0A";
	}
	if (isset($data['link']) && !empty($data['link'])) {
		$txt .= "Посилання на сайт або магазин: " . strip_tags(trim(urlencode($data['link']))) . "%0A";
	}
	if (isset($data['goal'][0]) && !empty($data['goal'][0])) {
		$txt .= "Мета: " . strip_tags(trim(urlencode($data['goal'][0]))) . "%0A";
	}
...
function sendTelegram($txt){
	...
	if (isset(json_decode($textSendStatus)->{'ok'}) && json_decode($textSendStatus)->{'ok'}) {
		if (!empty($_FILES['files']['tmp_name'])) {
			...
			$яFile =  "https://api.telegram.org/bot" . TOKEN . "/sendMediaGroup";
